...
class Analysis:
    ...
    def __init__(self, list_g1, list_g2, len1):
        ...
        # self.len_g1 = len(list_g1)
        # self.len_g2 = len(list_g2)
    ...
    def get_tube(self, index1, len_gene1, len_gene2):
        # 找到能与之对应的
        tem_err_list = []
        ...
        if index1 < len_gene1:
            ...
            if index1 == len_gene1 + 1:
                tem_err_list.append([index1, index1 - len_gene1])
            elif index1 == len_gene1 + len_gene2 and self.len1 % 2 == 0:
    ...
    def analysis_two(self):
        ...
        for k, v in new_conc:
            if k.count("+") == 1 and v > 2e-9:  # TODO 将浓度换成输入的浓度
                ...
                if abs(t1 - t2) - len(self.list_g1) not in [0, 1]:  # 错配
                    error.append(k)
                    # TODO 添加验证
                    print("验证:{0},{1}".format(t1, t2))
                    tem_err_list = self.get_tube(t1, len(self.list_g1), len(self.list_g2))
                    tem_err_list = tem_err_list + self.get_tube(t2, len(self.list_g1), len(self.list_g2))
                    # tem_err_list.append([t2, t2 - len(gene_list1) - 1, t2 - len(gene_list1)])

                    if self.verification_two(gene_list, tem_err_list):
                        error_end.append(k)
        # if len(error_end):
        #     return False
        print("目标{0},list1:{1},list2:{2}".format(self.len1, len(self.list_g1), len(self.list_g2)))
        print("出错的{0}".format(error))
        print("浓度大于1e-9数{0}".format(k_cou))
        print('总数{0}'.format(len(new_conc)))

    def verification_two(self, gene_list, list1):
        # 验证错配的是否有问题
        # print(list1)
        set1 = set()
        strands = {}
        for i in range(len(list1)):
            for j in range(len(list1[i])):
                if j == 0:
                    set1.add(list1[i][j])
                    strands[
                        Strand(gene_list[list1[i][j] - 1], name="E:{1}:{0}".format(list1[i][j], i))] = 2e-8  # TODO 改正浓度
    ...
    def analysis_three(self  ):
        ...
        for k, v in new_conc:
            if k.count("+") == 2 and v > 1e-9:  # TODO 将浓度换成输入的浓度
                ...
                if set_t != {1, len(self.list_g1) + 1, len(self.list_g1) + 2}:
                    error.append(k)
                    # 添加验证
                    print("验证:{0},{1},{2}".format(t1, t2, t3))
                    tem_err_list = self.get_tube(t1, len(self.list_g1), len(self.list_g2))
                    tem_err_list = tem_err_list + self.get_tube(t2, len(self.list_g1), len(self.list_g2))
                    tem_err_list = tem_err_list + self.get_tube(t3, len(self.list_g1), len(self.list_g2))

                    self.verification_two(gene_list, tem_err_list)

        print("目标:{0},list1:{1},list2:{2}".format(int(self.len1 / 2), len(self.list_g1), len(self.list_g2)))
    ...
    def analysis_all(self):
        count = len(self.list_g1) + len(self.list_g2)
